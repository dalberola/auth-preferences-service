# Documentation

Reference docs for **auth-preferences-service**. Start with the root
[README.md](../README.md) for setup; agents should read [CLAUDE.md](../CLAUDE.md).

| Doc | Covers |
| --- | --- |
| [architecture.md](architecture.md) | System design, components, request/auth flows |
| [api.md](api.md) | Full HTTP endpoint reference with examples |
| [data-model.md](data-model.md) | Collections, schemas, indexes, TTL |
| [configuration.md](configuration.md) | Environment variables, precedence, secrets |
| [development.md](development.md) | Docker workflow, scripts, testing, troubleshooting |
| [security.md](security.md) | Threat model and the controls that address it |
